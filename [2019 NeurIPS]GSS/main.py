...
class Continuum:
    ...
    def __init__(self, data, args, transform=None):
        ...
        n_tasks = len(data) # task 수
        task_permutation = range(n_tasks)

        if args.shuffle_tasks == 'yes': # shuffle No
        ...
        for t in range(n_tasks):
            # N -> task에 할당된 모든 class의 데이터 수
            N=data[t][1].size(0)    # t번째 task에 할당된 데이터 수
            print("Task per Class : ", N)
            if len(samples_per_task)>t :    # task가 처음인 경우
                if int(samples_per_task[t])<= 0:
                    n = N
                else:   # samples_per_task 값을 설정한 경우 사용, 대부분 이 부분 사용
                    n = min(int(samples_per_task[t]),N)
            else:   # task가 처음이 아닌 경우 -> 왜 task별 작게 할당된 samples로 바꾸는지?????
                n = min(n, N)
            print("*********Task",t,"Samples are",n)
            p = torch.randperm(data[t][1].size(0))[0:n] # index 순서로 셔플
            
            sample_permutations.append(p)   # 각 task 셔플 데이터셋

        self.permutation = []   # Permutation된 각 데이터의 index 값 저장

        for t in range(n_tasks):
            task_t = task_permutation[t]

            for _ in range(args.n_epochs):  # n_epochs = 1(default)
                task_p = [[task_t, i] for i in sample_permutations[task_t]]
                random.shuffle(task_p)    # permutation -> index shuffle
                self.permutation += task_p

        self.length = len(self.permutation) # 각 task별 n의 합
        self.current = 0
...
class Dataset(Dataset):
    def __init__(self, image, image_path, transform):
    ...
    def __getitem__(self, idx):
        image = cv2.imread(self.image_path + self.image_list[idx])
...
if __name__ == "__main__":
    ...
    if args.output_name:
        ...
        model.fname = args.model+'_D'+str(args.samples_per_task)+'_Mb'+str(args.n_sampled_memories) + '_Ma' +str(args.n_constraints) +'_St'+str(args.memory_strength)+ '_Ch'+str(args.change_th) + '_nit' +str(args.n_iter) +'_slack'+str( args.slack) +'_normalize'+str( args.normalize)+ '_'+ args.data_file + '_'
